...
def benchmark(args, pipeline_args, model_args, optimizer_args, dataset_args):
    ...
    start_points = sdfoam.nn(points, model.aabb_tree, positions)
    ...
    trace_kwargs = {"weight_threshold": 0.05}
    if model_args.use_sdfoam:
        inv_s = model.variance_net(
            torch.zeros([1, 3], device=model.device)
        )[:, :1].clip(1e-6, 1e6)
        trace_kwargs["alpha_mode"] = "sdfoam"
        trace_kwargs["inv_s"] = inv_s.reshape(())
