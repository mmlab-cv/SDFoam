...
class DataHandler:
    ...
    def reload(self, split, downsample=None):
        ...
        if self.points3D is not None:
            print(
                f"self.points min/max: {self.points3D.min(0).values}/{self.points3D.max(0).values}"
            )
        else:
            print("No sparse points3D available for this dataset.")
    ...
    def get_iter(self):
        ...
        alpha_batch_fetcher = sdfoam.BatchFetcher(
