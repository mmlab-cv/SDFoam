...
sdfoam::CMapTable upload_cmap_data() {
    sdfoam::CMapTable cmap_table;
...
struct ViewerPrivate : public Viewer {
    ...
    std::vector<uint8_t> attrs_cpu;
    std::vector<uint8_t> point_adjacency_cpu;
    std::vector<uint8_t> point_adjacency_offsets_cpu;
    std::vector<uint8_t> aabb_tree_cpu;
    std::vector<uint8_t> sharpness_cpu;
    CUDAArray<uint8_t> sharpness_buffer;
    CUDAArray<uint32_t> output_rgba_buffer;
    CUDAArray<uint32_t> start_index_buffer;
    bool scene_upload_pending = false;
    ...
    ViewerPrivate(std::shared_ptr<Pipeline> pipeline, ViewerOptions options)
        ...
        window =
            glfwCreateWindow(1920, 1080, "SDFoam Viewer", nullptr, nullptr);
        if (!window) {
            glfwTerminate();
    ...
    void run() {
        ...
        // Toggle NeuS vs legacy based on ViewerOptions
        if (options.use_sdfoam) {
            settings.alpha_mode  = AlphaNeuS;
            settings.inv_s       = options.inv_s;
            settings.eps         = options.eps;
            settings.sdf_channel = (options.sdf_channel >= 0)
                                    ? options.sdf_channel
                                    : int(pipeline->attribute_dim()) - 1;
        } else {
            settings.alpha_mode  = AlphaStandard;  // legacy Beer–Lambert
            // inv_s/eps/sdf_channel ignored in legacy mode
        }
        ...
        while (!glfwWindowShouldClose(window)) {
            ...
            // ImGui::SeparatorText("Alpha model");
            // ImGui::Checkbox("Use NeuS alpha", &options.use_sdfoam);
            // if (options.use_sdfoam) {
            //     ImGui::SliderFloat("inv_s (1/beta)", &options.inv_s, 1.0f, 200.0f, "%.1f", ImGuiSliderFlags_Logarithmic);
            //     ImGui::SliderFloat("eps", &options.eps, 1e-7f, 1e-3f, "%.1e", ImGuiSliderFlags_Logarithmic);
            //     int last = int(pipeline->attribute_dim()) - 1;
            //     ImGui::SliderInt("SDF channel", &options.sdf_channel, -1, last, "%d");
            // }


            ImGui::SeparatorText("Visualization settings");

            const char *modes[] = {"RGB", "Depth", "Alpha", "Intersections", "SDF"};
            ImGui::Combo("Mode",
            ...
            if (vis_settings.mode == VisualizationMode::Depth ||
                ...
                vis_settings.mode == VisualizationMode::SDF) {
            ...
            // SDF uses the same max_depth field to mean "max |SDF|"
            if (vis_settings.mode == VisualizationMode::SDF) {
                const auto logFlags =
                    ImGuiSliderFlags_Logarithmic | ImGuiSliderFlags_NoRoundToFormat;

                // Toggle between the two behaviors
                ImGui::Checkbox("Exact match (± tol)", &settings.sdf_exact_mode);

                if (settings.sdf_exact_mode) {
                    ImGui::SliderFloat("SDF value",
                        &settings.sdf_value, -10.f, 10.f, "%.4f");
                    ImGui::SliderFloat("Tolerance (>=0)",
                        &settings.sdf_tolerance, 0.0f, 5.f, "%.4f");
                    // clamp to non-negative
                    settings.sdf_tolerance = std::max(0.0f, settings.sdf_tolerance);
                } else {
                    ImGui::SliderFloat("Max |SDF|",
                        &settings.max_sdf, 1e-5f, 1e2f, "%.4f", logFlags);
                }
            }
            ...
            if (num_point_adjacency > 0) {
                if (scene_upload_pending) {
                    points_buffer.resize(points_cpu.size());
                    attrs_buffer.resize(attrs_cpu.size());
                    point_adjacency_buffer.resize(point_adjacency_cpu.size());
                    point_adjacency_offsets_buffer.resize(
                        point_adjacency_offsets_cpu.size());
                    adjacent_diff_buffer.resize((num_point_adjacency + 32) * 8);
                    sharpness_buffer.resize(sharpness_cpu.size());
                    ...
                    cuda_check(cuMemcpyHtoD(
                        ...
                        attrs_cpu.size()));
                    cuda_check(cuMemcpyHtoD(
                        (CUdeviceptr)point_adjacency_buffer.begin(),
                        point_adjacency_cpu.data(),
                        point_adjacency_cpu.size()));
                    cuda_check(cuMemcpyHtoD(
                        (CUdeviceptr)point_adjacency_offsets_buffer.begin(),
                        point_adjacency_offsets_cpu.data(),
                        point_adjacency_offsets_cpu.size()));
                    cuda_check(cuMemcpyHtoD(
                        (CUdeviceptr)sharpness_buffer.begin(),
                        sharpness_cpu.data(),
                        sharpness_cpu.size()));

                    prefetch_adjacent_diff(
                        reinterpret_cast<const Vec3f *>(points_buffer.begin()),
                        num_points,
                        num_point_adjacency,
                ...
                if (vis_settings.mode == VisualizationMode::RGB) {
                    size_t num_pixels =
                        static_cast<size_t>(camera.width) * camera.height;
                    size_t buffer_size = num_pixels * sizeof(uint32_t);
                    output_rgba_buffer.resize(num_pixels);
                    start_index_buffer.resize(1);
                    if (cpu_fallback_buffer.size() != buffer_size) {
                        cpu_fallback_buffer.resize(buffer_size);
                    }

                    cuda_check(cuMemcpyHtoD(
                        reinterpret_cast<CUdeviceptr>(start_index_buffer.begin()),
                        &start_index,
                        sizeof(uint32_t)));

                    pipeline->trace_benchmark(
                        settings,
                        num_points,
                        reinterpret_cast<const Vec3f *>(points_buffer.begin()),
                        attrs_buffer.begin(),
                        ...
                        vis_settings,
                        camera,
                        start_index_buffer.begin(),
                        output_rgba_buffer.begin(),
                        sharpness_buffer.begin());

                    cuda_check(cuStreamSynchronize(0));
                    cuda_check(cuMemcpyDtoH(
                        cpu_fallback_buffer.data(),
                        reinterpret_cast<CUdeviceptr>(output_rgba_buffer.begin()),
                        buffer_size));
    ...
    void update_scene(uint32_t num_points,
                      ...
                      const void *sharpness_host) override {
        scene_updating = true;
        {
            ...
            attrs_cpu.resize(attr_bytes);
            point_adjacency_cpu.resize(point_adjacency_bytes);
            point_adjacency_offsets_cpu.resize(point_adjacency_offsets_bytes);
            aabb_tree_cpu.resize(aabb_tree_bytes);
            sharpness_cpu.resize(num_points * sizeof(float));

            cuda_check(cuMemcpyDtoH(points_cpu.data(),
                                    (CUdeviceptr)coords,
                                    coord_bytes));
            cuda_check(cuMemcpyDtoH(attrs_cpu.data(),
                                    (CUdeviceptr)attributes,
                                    attr_bytes));
            cuda_check(cuMemcpyDtoH(point_adjacency_cpu.data(),
                                    (CUdeviceptr)point_adjacency,
                                    point_adjacency_bytes));
            cuda_check(cuMemcpyDtoH(point_adjacency_offsets_cpu.data(),
                                    (CUdeviceptr)point_adjacency_offsets,
                                    point_adjacency_offsets_bytes));
            cuda_check(cuMemcpyDtoH(
                aabb_tree_cpu.data(), (CUdeviceptr)aabb_tree, aabb_tree_bytes));
            cuda_check(cuMemcpyDtoH(sharpness_cpu.data(),
                                    (CUdeviceptr)sharpness_host,
                                    sharpness_cpu.size()));

            scene_upload_pending = true;
            scene_updating = false;
        }
        scene_cv.notify_one();
    }
