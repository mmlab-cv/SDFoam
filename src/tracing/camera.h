...
struct Camera {
    ...
    SDFOAM_HD void rotate(Vec3f axis, float angle) {
...
inline SDFOAM_HD Camera look_at(const Vec3f &position,
                                 const Vec3f &target,
                                 const Vec3f &up,
                                 float fov,
                                 uint32_t width,
...
inline SDFOAM_HD Ray cast_ray(const Camera &camera, int i, int j) {
    Ray ray;
