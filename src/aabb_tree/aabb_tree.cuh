...
__forceinline__ __host__ __device__ uint32_t ctz32(uint32_t x) {
#ifdef __CUDA_ARCH__
    return __ffs(x) - 1;
...
traverse(uint32_t num_points, uint32_t max_depth, Functor functor) {
    ...
    for (;;) {
        ...
        current_node++;
        uint32_t step_up_amount = ctz32(current_node);
        step_up_amount =
            step_up_amount < current_depth ? step_up_amount : current_depth;
