...
struct PODMat {
    ...
    SDFOAM_HD PODMat(const Eigen::Matrix<scalar, rows, cols> &mat) {
        memcpy(data, mat.data(), sizeof(data));
    }

    SDFOAM_HD Eigen::Matrix<scalar, rows, cols> &operator*() {
        return *reinterpret_cast<Eigen::Matrix<scalar, rows, cols> *>(data);
    }

    SDFOAM_HD const Eigen::Matrix<scalar, rows, cols> &operator*() const {
...
inline SDFOAM_HD Vec3f rand_unit3(RNGState &rngstate) {
...
SDFOAM_HD bool set_equal(const T *a, const T *b) {
    bool equal = true;
#pragma unroll
    for (int i = 0; i < n; i++) {
        bool found = false;
...
template <int n, typename T>
SDFOAM_HD bool set_contains(const T *a, const T &b) {
    bool found = false;
#pragma unroll
    for (int i = 0; i < n; i++) {
        found |= (a[i] == b);
...
template <int n, typename T>
SDFOAM_HD void sort_in_place(T *a) {
#pragma unroll
    for (int i = 0; i < n; i++) {
#pragma unroll
        for (int j = i + 1; j < n; j++) {
...
template <int n, typename T>
SDFOAM_HD bool set_less(const T *a, const T *b) {
    T a_sorted[n];
    T b_sorted[n];
...
template <int n, typename T>
SDFOAM_HD bool permutation_parity(const T *perm) {
    int parity = 0;
#pragma unroll
    for (int i = 0; i < n; i++) {
#pragma unroll
...
SDFOAM_HD Iter binary_search(Iter begin,
                              Iter end,
...
inline SDFOAM_HD uint32_t morton_code(const Vec3i &coords) {
    uint32_t code = 0;
#pragma unroll
...
inline SDFOAM_HD Vec3i inverse_morton_code(uint32_t code) {
    Vec3i coords(0, 0, 0);
#pragma unroll
    for (uint32_t i = 0; i < 10; i++) {
...
struct AABB {
    ...
    /// sphere.
    SDFOAM_HD IntersectionResult intersects_sphere(const Vec3<scalar> &center,
                                                    scalar radius) const {
...
struct IndexedTriangle {
    ...
    SDFOAM_HD IndexedTriangle() {}

    SDFOAM_HD IndexedTriangle(uint32_t i0, uint32_t i1, uint32_t i2) {
        vertices[0] = i0;
        vertices[1] = i1;
        vertices[2] = i2;
    }
    ...
    SDFOAM_HD friend bool operator==(const IndexedTriangle &a,
    ...
    template <typename scalar>
    SDFOAM_HD Vec3<scalar> normal(const Vec3<scalar> &v0,
    ...
    template <typename scalar>
    SDFOAM_HD Vec3f barycentric_coords(const Vec3<scalar> *points,
...
struct IndexedTet {
    ...
    SDFOAM_HD IndexedTet() {}

    SDFOAM_HD IndexedTet(uint32_t i0, uint32_t i1, uint32_t i2, uint32_t i3) {
        vertices[0] = i0;
        vertices[1] = i1;
        vertices[2] = i2;
        vertices[3] = i3;
    ...
    SDFOAM_HD friend bool operator==(const IndexedTet &a,
                                      const IndexedTet &b) {
        return set_equal<4>(a.vertices, b.vertices);
    }

    SDFOAM_HD friend bool operator<(const IndexedTet &a, const IndexedTet &b) {
    ...
    SDFOAM_HD IndexedTriangle face(uint32_t i) const {
        switch (i % 4) {
        case 0:
    ...
    template <typename scalar>
    SDFOAM_HD scalar volume(const Vec3<scalar> &v0,
                             const Vec3<scalar> &v1,
...
struct IndexedEdge {
    ...
    SDFOAM_HD IndexedEdge() {}

    SDFOAM_HD IndexedEdge(uint32_t i0, uint32_t i1) {
        vertices[0] = i0;
        vertices[1] = i1;
    }
    ...
    SDFOAM_HD friend bool operator==(const IndexedEdge &a,
                                      const IndexedEdge &b) {
